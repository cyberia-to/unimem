...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let pairs = [
        ...
        ("ANE: compile+load", ref_result.ane_compile, cyb_result.ane_compile),
...
fn run_standard() -> Result<LayerResult, Box<dyn std::error::Error>> {
    ...
    for tok in 0..SEQ {
        ...
        acpu::vector::rmsnorm(&mut norm_out, row, &rms_weight, 1e-5);
    ...
    // AMX: QKV projection
    let t = Instant::now();
    acpu::sgemm(&x, &wq, &mut q, SEQ, DIM, DIM);
    acpu::sgemm(&x, &wk, &mut k, SEQ, DIM, DIM);
    acpu::sgemm(&x, &wv, &mut v, SEQ, DIM, DIM);
    let t_qkv = t.elapsed();
    ...
    for tok in 0..SEQ {
        ...
        acpu::vector::rope(&mut q_tok, &q[tok * DIM..tok * DIM + HEAD_DIM], &freqs, tok);
    ...
    let t = Instant::now();
    acpu::sgemm(&q[..SEQ * HEAD_DIM], &k[..SEQ * HEAD_DIM], &mut scores, SEQ, SEQ, HEAD_DIM);
    ...
    let t = Instant::now();
    acpu::sgemm(&scores, &v[..SEQ * HEAD_DIM], &mut attn_out[..SEQ * HEAD_DIM], SEQ, HEAD_DIM, SEQ);
    let t_attn_v = t.elapsed();

    // AMX: FFN up + gate
    let t = Instant::now();
    acpu::sgemm(&x, &w_up, &mut ffn_up_out, SEQ, FFN_DIM, DIM);
    acpu::sgemm(&x, &w_gate, &mut ffn_gate_out, SEQ, FFN_DIM, DIM);
    let t_ffn_up = t.elapsed();

    // CPU: SiLU(gate) * up
    let t = Instant::now();
    acpu::vector::silu(&mut ffn_gate_out);
    for i in 0..ffn_up_out.len() { ffn_up_out[i] *= ffn_gate_out[i]; }
    let t_silu = t.elapsed();

    // AMX: FFN down
    let t = Instant::now();
    acpu::sgemm(&ffn_up_out, &w_down, &mut ffn_out, SEQ, DIM, FFN_DIM);
    let t_ffn_down = t.elapsed();

    // ANE: small matmul
    let t = Instant::now();
    let program = rane::mil::matmul(64, 64, 64);
    let mut model = rane::AneModel::compile(&program, &[])?;
    model.load()?;
    let t_ane_compile = t.elapsed();

    let ane_in = rane::AneSurface::new(program.input_bytes())?;
    let ane_out = rane::AneSurface::new(program.output_bytes())?;
    ...
    let ane_ok = ane_out.with_data(|d| {
    ...
    Ok(LayerResult {
        alloc: t_alloc, rmsnorm: t_rmsnorm, qkv: t_qkv, rope: t_rope,
        attn: t_attn, softmax: t_softmax, attn_v: t_attn_v,
        ffn_up: t_ffn_up, silu: t_silu, ffn_down: t_ffn_down,
        ane_compile: t_ane_compile, ane_run: t_ane_run,
        total: t_total, pass,
    })
}
...
fn run_cybmem() -> Result<LayerResult, Box<dyn std::error::Error>> {
    ...
    x.fill(0.01);
    rms_weight.fill(1.0);
    wq.fill(0.01); wk.fill(0.01); wv.fill(0.01);
    w_up.fill(0.01); w_gate.fill(0.01); w_down.fill(0.01);
    freqs.fill(1.0);
    let t_alloc = t.elapsed();
    ...
    for tok in 0..SEQ {
        let row = &x[tok * DIM..(tok + 1) * DIM];
        acpu::vector::rmsnorm(norm_out, row, rms_weight, 1e-5);
        x[tok * DIM..(tok + 1) * DIM].copy_from_slice(norm_out);
    }
    let t_rmsnorm = t.elapsed();
    ...
    // AMX: QKV
    let t = Instant::now();
    acpu::sgemm(x, wq, q, SEQ, DIM, DIM);
    acpu::sgemm(x, wk, k, SEQ, DIM, DIM);
    acpu::sgemm(x, wv, v, SEQ, DIM, DIM);
    let t_qkv = t.elapsed();
    ...
    // AMX: attention
    let t = Instant::now();
    acpu::sgemm(&q[..SEQ * HEAD_DIM], &k[..SEQ * HEAD_DIM], scores, SEQ, SEQ, HEAD_DIM);
    let t_attn = t.elapsed();
    ...
    // AMX: FFN
    let t = Instant::now();
    acpu::sgemm(x, w_up, ffn_up_out, SEQ, FFN_DIM, DIM);
    acpu::sgemm(x, w_gate, ffn_gate_out, SEQ, FFN_DIM, DIM);
    let t_ffn_up = t.elapsed();
    ...
    // AMX: FFN down
    let t = Instant::now();
    acpu::sgemm(ffn_up_out, w_down, ffn_out, SEQ, DIM, FFN_DIM);
    let t_ffn_down = t.elapsed();
    ...
    let cyb_in = unimem::Block::open(program.input_bytes())?;
    let cyb_out = unimem::Block::open(program.output_bytes())?;
    fill_ane_identity_raw(&cyb_in, &program);

    let t = Instant::now();
    unsafe { model.run_raw(cyb_in.handle(), cyb_out.handle())? };
    let t_ane_run = t.elapsed();

    let ane_ok = unsafe {
        let d = std::slice::from_raw_parts(cyb_out.address() as *const u16, program.output_bytes() / 2);
        let (oc, osp) = program.output_shape();
        d[..oc * osp].iter().all(|&v| rane::fp16_to_f32(v) == 1.0)
    };
    model.unload()?;
    ...
    println!("  alloc (tape): {:?}  ({:.1} MB used / {:.1} MB cap)",
             t_alloc, tape.used() as f64 / 1e6, tape.total() as f64 / 1e6);
    println!("  CPU rmsnorm:   {:?}", t_rmsnorm);
    println!("  AMX qkv:       {:?}", t_qkv);
    println!("  CPU rope:      {:?}", t_rope);
    println!("  AMX attn:      {:?}", t_attn);
...
fn fill_ane_identity(surface: &rane::AneSurface, program: &rane::MilProgram) {
    let (_, in_sp) = program.input_shape();
    surface.with_data_mut(|data| {
        for ch in 0..64 {
            for s in 0..64 { data[ch * in_sp + s] = rane::f32_to_fp16(1.0); }
            for o in 0..64 { data[ch * in_sp + 64 + o] = if ch == o { rane::f32_to_fp16(1.0) } else { 0 }; }
        }
    });
}

fn fill_ane_identity_raw(surface: &unimem::Block, program: &rane::MilProgram) {
    let (_, in_sp) = program.input_shape();
    unsafe {
        let data = std::slice::from_raw_parts_mut(surface.address() as *mut u16, program.input_bytes() / 2);
        for ch in 0..64 {
            for s in 0..64 { data[ch * in_sp + s] = rane::f32_to_fp16(1.0); }
            for o in 0..64 { data[ch * in_sp + 64 + o] = if ch == o { rane::f32_to_fp16(1.0) } else { 0 }; }
        }
